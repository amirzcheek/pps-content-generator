...
[ Браузер ] → HTTPS → [ nginx основного домена (10.99.99.10, TLS портала) ]
                   ...
                   content-gen: FastAPI/uvicorn (React + API в одном сервисе)
                          │  LLM_BASE_URL
                          ▼
                   [ LLM-сервер (10.99.99.201) ] — локальная модель 14b
```
...
- Первая версия — русский/английский (модель 14b). Казахский (`KAZ_*`) — позже.
...
Заполнить в `.env`:
```ini
LLM_BASE_URL=http://10.99.99.201:<порт>/v1     # адрес локальной модели 14b
LLM_MODEL=<имя модели>
LLM_API_KEY=not-needed
KAZ_BASE_URL=                                   # казахский — позже
KAZ_MODEL=
KAZ_API_KEY=
